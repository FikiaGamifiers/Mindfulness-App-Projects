...
class ConflictApp(QWidget):
    ...
    def show_results(self):
        ...
        for i in reversed(range(self.layout.count())):
            ...
            if widget is not None and widget is not the self.next_button:
                widget.deleteLater()
    ...
    def display_chart(self):
        ...
        styles = list(self.responses.keys())
        scores = list(self.responses.values())
        colors = ['#FF5C8D', '#FF6F61', '#6A5ACD', '#4682B4']  # Example colors for styles

        ax.barh(styles, scores, color=colors)
        ax.set_xlabel('Scores')
        ax.set_title('Conflict Style Scores')
